...
with
    all_events as (select * from {{ ref("snowplow_base_events") }}),
    events as (
    ...
    web_page_context as (select * from {{ ref("snowplow_web_page_context") }}),
    prep as (
        ...
        select
            ...
            ev.os_name,
            ev.os_family,
            ev.os_manufacturer, replace (ev.os_timezone, '%2F', '/') as os_timezone,
        ...
        where ev.platform = 'web' and ev.event_name = 'page_view'

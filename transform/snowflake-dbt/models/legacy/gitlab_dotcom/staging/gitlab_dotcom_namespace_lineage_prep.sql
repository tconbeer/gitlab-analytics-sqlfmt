...
recursive_namespaces(namespace_id, parent_id, upstream_lineage) as (
    ...
    select
        namespace_id,
        parent_id,
        to_array(namespace_id)
        as upstream_lineage  -- Initiate lineage array
    from namespaces
    ...
    select
        iter.namespace_id,
        iter.parent_id,
        array_append(anchor.upstream_lineage, iter.namespace_id)
        as upstream_lineage  -- Copy the lineage array of parent, append self to end
    from
        recursive_namespaces
        as anchor  -- Parent namespace
    inner join
        namespaces
        as iter  -- Child namespace
...
extracted as (
    ...
    select
        ...
        as ultimate_parent_id,  -- First item is the ultimate parent.
        iff(

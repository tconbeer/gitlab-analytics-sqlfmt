...
with
    ...
    consolidated_accounts as (
        ...
        select
            ...
            case
                when a.technical_account_manager is not null then 1 else 0
            ...
            case
                when coalesce(arr.arr, 0) > 50000 then 1 else 0
            end as is_over_50k_customer_flag,

            case
                when coalesce(arr.arr, 0) > 100000 then 1 else 0
            end as is_over_100k_customer_flag,

            case
                when coalesce(arr.arr, 0) > 500000 then 1 else 0
            ...
            coalesce(net_arr_ttm.ttm_channel_net_arr, 0) - coalesce(
                net_arr_ttm.ttm_channel_sourced_net_arr, 0
    ...
    upa_lam as (
        ...
        select
            ...
            case
                when potential_users > licenses then potential_users else 0
            end as adjusted_potential_users,

            case
                when linkedin_developer > licenses then linkedin_developer else 0
            end as adjusted_linkedin_developers,

            case
                when zi_developers > licenses then zi_developers else 0
            end as adjusted_zi_developers,

            case
                when employees * 0.1 > licenses then round(employees * 0.1, 0) else 0

...
dim_date_extended as (
    ...
    select dim_date.*, current_fiscal_year.current_fiscal_year
    from dim_date
...
final as (
    ...
    select
        ...
        -- If the fiscal year of the SAO date is lower than the current fiscal year,
        -- use the sales hierarchy from the account owner
        ...
        iff(
            dim_date_sao_date.fiscal_year < dim_date_sao_date.current_fiscal_year,
        ...
        -- If the fiscal year of close date is lower than the current fiscal year, use
        -- the sales hierarchy from the account owner
        -- If not, use the stamped hierarchy
        dim_crm_opportunity.crm_opp_owner_stamped_name,
        dim_crm_opportunity.crm_account_owner_stamped_name,

        iff(
            dim_date_close_date.fiscal_year < dim_date_close_date.current_fiscal_year,
            crm_account_user_sales_segment,
    ...
    left join
        dim_date_extended as dim_date_close_date
        on fct_crm_opportunity.close_date = dim_date_close_date.date_day
    left join
        dim_date_extended as dim_date_sao_date

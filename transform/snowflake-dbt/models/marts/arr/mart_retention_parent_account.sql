...
next_renewal_month as (
    ...
    select distinct
        ...
        min(subscription_end_month) over (
            partition by merged_accounts.dim_parent_crm_account_id
...
parent_account_mrrs as (
    ...
    select
        ...
        last_renewal_month,
        count(
            distinct dim_crm_account.dim_parent_crm_account_id
    ...
    left join
        ...
        = dim_crm_account.dim_parent_crm_account_id
    where dim_crm_account.is_jihu_account != 'TRUE' {{ dbt_utils.group_by(n=5) }}
    ...
    select
        {{
            dbt_utils.surrogate_key(
                ["retention_subs.dim_parent_crm_account_id", "retention_month"]
            )
        }} as fct_retention_id,
        ...
        coalesce(
            future_parent_customer_count, 0
        ) as net_retention_parent_customer_count,
        {{

...
as (  -- get the base data set of recurring charges.
...
agg_charge_term_less_than_equal_12_{{ renewal_fiscal_year }}
...
agg_charge_term_greater_than_12_{{ renewal_fiscal_year }}
...
twenty_four_mth_term_{{ renewal_fiscal_year }}
...
thirty_six_mth_term_{{ renewal_fiscal_year }}
...
forty_eight_mth_term_{{ renewal_fiscal_year }}
-- create records for the intermitent renewals for multi-year bookings that are not in
-- the Zuora data. The start and end months are in the agg_MYB for multi-year
-- bookings.
as (
...
sixty_mth_term_{{ renewal_fiscal_year }}
-- create records for the intermitent renewals for multi-year bookings that are not in
-- the Zuora data. The start and end months are in the agg_MYB for multi-year
-- bookings.
as (
...
combined_{{ renewal_fiscal_year }}
as (  -- union all of the charges
...
renewal_report_{{ renewal_fiscal_year }}
as (  -- create the renewal report for the applicable fiscal year.

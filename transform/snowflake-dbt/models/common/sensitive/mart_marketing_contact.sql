...
flattened_members as (
    ...
    inner join
        ...
        on members.source_id
        = namespaces.dim_namespace_id  -- same as namespace_id for group namespaces
    where
        lower(members.member_source_type)
        = 'namespace'  -- only looking at namespace invites
        and members.invite_created_at
        >= namespaces.namespace_created_at  -- invite created after namespace created
...
invite_status as (
    ...
    select  -- pull in relevant namespace data, invite status, etc
        ...
        members.user_id,
        iff(memberships.user_id is not null, true, false)
        -- flag whether the user actually joined the namespace
        as invite_was_successful
    ...
    where
        members.user_id
        != namespaces.creator_id  -- not an "invite" if user created namespace
...
subscriptions as (
    ...
    where
        ...
        and charges.product_category in (
            'SaaS - Ultimate', 'SaaS - Premium'
...
stages_adopted as (
    ...
    where
        ...
        and events.plan_name_at_event_date in (
            'trial', 'free', 'ultimate_trial'

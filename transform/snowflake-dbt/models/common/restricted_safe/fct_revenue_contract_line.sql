with
    ...
    dim_date as (select * from {{ ref("date_details_source") }}),
    zuora_account as (select * from {{ ref("zuora_account_source") }}),
    prep_quote as (select * from {{ ref("prep_quote") }}),
    final as (
        ...
        select
            ...
            -- pob ids
            lines_source.revenue_contract_performance_obligation_id
            ...
            {{ get_date_id("lines_source.original_fair_value_date") }}
            as original_fair_value_date_id,
            {{ get_date_id("lines_source.fair_value_expiration") }}
            as fair_value_expiration_id,
            {{ get_date_id("lines_source.forecast_date") }} as forecast_date_id,
            {{ get_date_id("lines_source.unbilled_reversal_period") }}
            ...
            {{ get_date_id("lines_source.revenue_contract_line_created_date") }}
            as revenue_contract_line_created_date_id,
            {{ get_date_id("lines_source.revenue_contract_line_updated_date") }}
            as revenue_contract_line_updated_date_id,
            {{ get_date_id("lines_source.incremental_update_date") }}
        ...
        left join
            pob
            on lines_source.revenue_contract_performance_obligation_id
            = pob.revenue_contract_performance_obligation_id
        left join
            zuora_account on lines_source.billing_account_id = zuora_account.account_id
        left join prep_quote on lines_source.quote_number = prep_quote.quote_number

with
    ...
    renamed as (
        ...
        select
            ...
            regexp_replace(total_touch_billings, '[,]', '')::number
            as total_touch_billings,
            regexp_replace(total_beginning_ar, '[,]', '')::number as total_beginning_ar,
            regexp_replace(total_ar_at_end_of_period, '[,]', '')::number
            as total_ar_at_end_of_period,
            nbr_of_days_in_period::number as nbr_of_days_in_period,
            dso::number as dso,
            regexp_replace(collections_based_on_formula, '[,]', '')::number
            as collections_based_on_formula,
            regexp_replace(total_current_ar_close, '[,]', '')::number
            as total_current_ar_close,

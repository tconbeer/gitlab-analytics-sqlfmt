with
    ...
    renamed as (
        ...
        select
            ...
            )::float as number_of_open_opportunities,
            nullif(
                "Count_of_Won_Opportunities", ''
            )::float as count_of_won_opportunities,
            nullif(
                "Total_Closed_Won_Amount_(All-Time)", ''
            )::float as total_closed_won_amount,
            nullif(
                "Sum:_Open_New_Add-on_IACV_Opportunities", ''
            )::float as open_new_add_on_iacv,
            nullif(
                "Sum_of_Open_Renewal_Opportunities", ''
            )::float as sum_open_renewal_opportunities,
            ...
            nullif(
                "Concurrent_EE_Subscriptions", ''
            )::float as concurrent_ee_subscriptions,
            nullif(
                "Count_of_Active_Subscriptions", ''
            )::float as count_active_subscriptions,
            ...
            nullif(
                "DemandBase:_Trending_Onsite_Engagement", ''
            )::float as demandbase_trending_onsite_engagement,
            nullif(
                "DemandBase:_Trending_Offsite_Intent", ''
            )::float as demandbase_trending_offsite_intent,

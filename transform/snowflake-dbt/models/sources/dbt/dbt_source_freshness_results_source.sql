...
with
    ...
    v0parsed as (
        ...
        where
            ...
            and s.value['state']::varchar
            -- impossible to know what freshness is, so filtered out
            != 'runtime error'
            and s.value['max_loaded_at']::timestamp
            is not null  -- latest_load_at
            and s.value['snapshotted_at']::timestamp
            is not null  -- freshness_observed_at
            and s.value['max_loaded_at_time_ago_in_s']::float
            is not null  -- time_since_loaded_seconds

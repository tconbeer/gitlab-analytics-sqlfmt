...
create
or replace function "{{ target.database | trim }}_{{ db | trim }}".{{ target.schema }}.crc32(
    "input" string
)
...
grant usage
on function "{{ db | trim }}".{{ target.schema }}.crc32(string) to role public
;

{%- else -%}

grant usage
on function "{{ target.database | trim }}_{{ db | trim }}".{{ target.schema }}.crc32(
    string
) to role public
;

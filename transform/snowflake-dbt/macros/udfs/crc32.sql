{% macro crc32() %}
    ...
    {% for db in production_databases %}
        {%- if target.name in production_targets -%}
            ...
            create or replace function
                "{{ db | trim }}".{{ target.schema }}.crc32("input" string)

        {%- else -%}

            create or replace function
                "{{ target.database | trim }}_{{ db | trim }}".{{ target.schema }}.crc32(
                    "input" string
                )
    ...
    $$
        ...
        {%- if target.name in production_targets -%}
            ...
            grant usage
            on function "{{ db | trim }}".{{ target.schema }}.crc32(string)
            to role public
            ;
        ...
        {%- else -%}
            ...
            grant usage
            on
                function "{{ target.database | trim }}_{{ db | trim }}".{{ target.schema }}.crc32(
                    string
                )
            to role public
            ;

    ...
    windowed as (
        ...
        select
            ...
            coalesce(  -- First, look for the row immediately following by PK and subtract one millisecond from its timestamp.
                ...
                -- If row has no following rows, check when it's valid until (NULL if
                -- it appeared in latest task instance.)
                iff(
                    is_in_most_recent_task = false,
                    max_by_primary_key.max_timestamp,
                    null
                )
            ) as valid_to,
        ...
        left join
            max_by_primary_key
            on {{ casted_cte }}.{{ primary_key_renamed }}
            = max_by_primary_key.primary_key

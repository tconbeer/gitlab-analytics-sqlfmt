{%- macro sfdc_account_fields(model_type) %}
    ...
    with
        map_merged_crm_account as (
            ...
            select *
            from {{ ref("map_merged_crm_account") }} {%- if model_type == "live" %}
            ...
            {%- elif model_type == "snapshot" %}
                ...
                inner join
                    ...
                    and snapshot_dates.date_actual < coalesce(
                        sfdc_account_snapshots_source.dbt_valid_to,
        ...
        sfdc_users as (
            ...
            select
                ...
                {%- elif model_type == "snapshot" %}
                    {{
                        dbt_utils.surrogate_key(
                            [
                                "sfdc_user_snapshots_source.user_id",
                                "snapshot_dates.date_id",
                            ]
                        )
            ...
            {%- elif model_type == "snapshot" %}
                ...
                inner join
                    snapshot_dates
                    on snapshot_dates.date_actual
                    >= sfdc_user_snapshots_source.dbt_valid_from
                    and snapshot_dates.date_actual < coalesce(
        ...
        ultimate_parent_account as (
            ...
            select
                ...
                {%- elif model_type == "snapshot" %}
                    crm_account_snapshot_id, snapshot_id,
                {%- endif %}
                account_id,
                account_name,
                billing_country,
        ...
        final as (
            ...
            select
                ...
                case
                    when
                        lower(sfdc_account.gtm_strategy) in (
                            'account centric',
                ...
                sfdc_account.account_owner_user_segment
                as crm_account_owner_user_segment,
                ...
                as crm_account_zoom_info_dozisf_zi_id,
                ...
                case
                    when
                        lower(ultimate_parent_account.gtm_strategy) in (
                            'account centric',
                            'account based - net new',
                            'account based - expand'
                        )

...
case
    ...
    -- Minutes remaining in day + minutes elapsed over the weekend (2880) + unused SLA
    -- minutes
    when
        {{ ticket_priority }} = 'high' and dayofweek({{ created_at }}) = 5 and hour(
            {{ created_at }}
        ) >= 20 and {{ first_reply_time }} <= {{ minutes_before_day_end }} + 2880 + (
    ...
    when
        {{ ticket_priority }} = 'high' and dayofweek({{ created_at }}) = 6 or dayofweek(
            {{ created_at }}
        ) = 0 and {{ first_reply_time }} <= 2880 + 240
    then true
    ...
    -- minutes
    when
        {{ ticket_priority }} = 'normal' and dayofweek({{ created_at }}) = 5 and hour(
            {{ created_at }}
        ) >= 16 and {{ first_reply_time }} <= {{ minutes_before_day_end }} + 2880 + (
    ...
    when
        {{ ticket_priority }} = 'normal' and dayofweek(
            {{ created_at }}
        ) = 6 or dayofweek(
            {{ created_at }}
        ) = 0 and {{ first_reply_time }} <= 2880 + 480
    then true
    ...
    -- remaining in day + minutes elapsed over the weekend (2880) + unused SLA minutes
    when
        {{ ticket_priority }} = 'low' and dayofweek({{ created_at }}) = 5 and hour(
            {{ created_at }}
        ) > 0 and {{ first_reply_time }} <= {{ minutes_before_day_end }} + 2880 + (
    ...
    when
        {{ ticket_priority }} = 'low' and dayofweek({{ created_at }}) = 6 or dayofweek(
            {{ created_at }}
        ) = 0 and {{ first_reply_time }} <= 2880 + 1440
    then true

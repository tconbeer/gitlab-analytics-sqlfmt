{% macro rpt_ratio_sales_management_cut_generator(
    ...
    -- For the rpt_models align the column names and how Missing values are encoded
    -- with the null_or_missing macro.
    ...
    crm_person as (
        ...
        select
            ...
            {{
                null_or_missing(
                    "sales_segment_region_mapped", "segment_region_grouped"
                )
            }},
            {{
                null_or_missing(
                    "sales_qualified_source_name", "sales_qualified_source"
                )
            }},
    ...
    crm_opportunity_closed_period as (
        ...
        select
            ...
            {{
                null_or_missing(
                    "crm_opp_owner_sales_segment_stamped", "sales_segment"
                )
            }},
            {{
                null_or_missing(
    ...
    crm_opportunity_accepted_period as (
        ...
        select
            ...
            {{
                null_or_missing(
                    "crm_user_sales_segment_grouped", "sales_segment_grouped"
                )
            }}
    ...
    sales_funnel_target as (
        ...
        select
            {{
                dbt_utils.star(
                    from=ref("rpt_sales_funnel_target"),
    ...
    -- Union all the data sources columns to create a base list that can be used to
    -- join all the metrics too
    prep_base_list as (
    ...
    -- To the above base list add support for subtotals and a total column that will
    -- be joined later
    base_list as (
    ...
    -- Calculate the metrics in each CTE. Uses Group by ROLLUP to also calculate
    ...
    targets_full as (
        ...
        select
            ...
            sum(
                iff(kpi_name = 'Total Closed', allocated_target, 0)
            ) as "Total Opps / TQ"
    ...
    agg as (
        ...
        left join
            ...
            = new_logos_actual.fiscal_quarter_name_fy
            {% for select_column in select_columns %}
        ...
        {% if select_column == "segment_region_grouped" %}
            ...
            large_subtotal_new_logo as (
                ...
                where
                    ...
                    and is_closed = 'TRUE'
                    and is_edu_oss = 0
                    -- AND IFF([new_logos] = FALSE, TRUE, order_type = '1. New - First
                    -- Order')
                    and iff(
                        {{ is_new_logo_calc }} = false,
                        true,
            ...
            large_subtotal_mql_trial as (
                ...
                select
                    ...
                    count(
                        distinct iff(
                            is_lead_source_trial, email_hash || lead_source, null
                        )
                    ) as "Trials / A"
                from crm_person
                where
            ...
            large_subtotal as (
                ...
                left join
                    ...
                    {% for select_column in select_columns %}
                        {% if select_column == "segment_region_grouped" %}
                            and iff(
                                large_subtotal_mql_trial.segment_region_grouped
                                like 'Large%',
                                'Large-Total',
                                null
                            )

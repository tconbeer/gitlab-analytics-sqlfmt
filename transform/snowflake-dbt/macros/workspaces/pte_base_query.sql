    ...
    with
        ...
        period_1 as (
            ...
            select
                ...
                max(
                    parent_crm_account_tsp_sub_region
        ...
        target as (
            ...
            from  -- For accounts with multiple subscriptions we first have to sum their ARR to the arr_month level
                (
                    ...
                    from
                        prod.restricted_safe_common_mart_sales.mart_arr_snapshot_bottom_up  -- Contains Snapshot for every date from 2020-03-01 to Present
                    where
        ...
        -- Any metrics you would want to calculate the require multiple ARR_MONTHS.
        -- Could be lifetime metrics, over the last year, future expected ARR. etc.
        /*
), lifetime AS (
...
*/
        ...
        opps as (
            ...
            select
                ...
                sum(
                    case when deal_size = 'Other' then 1 else 0 end
                ) as deal_size_other_cnt,
                sum(
                ...
                sum(
                    case
                        when order_type_stamped = '1. New - First Order' then 1 else 0
                    end
                ...
                ) as competitors_amazon_code_commit,
                ...
                sum(
                    case
                        when cp_use_cases = 'CI: Automate build and test' then 1 else 0
                    end
                ...
                sum(
                    case when cp_use_cases = 'Other' then 1 else 0 end
                ) as use_case_other,
                sum(
                    case
                        when
        ...
        zi_technologies as (
            ...
            select
                ...
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: Atlassian') then 1
                    end
                ...
                max(
                    case
                        when
                            contains(
                                zi_technologies__c,
                                'ARE_USED: Atlassian Jira Agile Tools'
                            )
                        then 1
                    end
                ) as zi_jira_flag,
                    ...
                    case
                        when
                            (
                                contains(
                                    zi_technologies__c,
                                    'ARE_USED: Google Cloud Platform'
                                )
                ...
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: GitHub') then 1
                    end
                ) as zi_github_flag,
                max(
                    case
                ...
                max(
                    case
                        when
                            contains(
                                zi_technologies__c, 'ARE_USED: Amazon AWS CloudTrail'
                            )
                        then 1
                    end
                ...
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: Hashicorp') then 1
                    end
                    ...
                    case
                        when
                            (
                                ...
                                or contains(
                                    zi_technologies__c,
                                    'ARE_USED: Circle Internet Services'
                                )
                            )
                        then 1
                    end
                ...
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: TravisCI') then 1
                    end
                ) as zi_travisci_flag,
                -- Open Source/Free
                max(
                ...
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: Jenkins') then 1
                    end
                ) as zi_jenkins_flag,
                max(
                    case
                        when contains(zi_technologies__c, 'ARE_USED: TortoiseSVN')
                        then 1
                    end
        ...
        bizible as (
            ...
            select
                ...
                sum(
                    case
                        when bizible_touchpoint_source = 'Organic - Google'
                        then 1
                        else 0
                    end
                ...
                sum(
                    case
                        when bizible_touchpoint_source = 'CRM Campaign' then 1 else 0
                    end
                ...
                sum(
                    case
                        when bizible_touchpoint_source = 'CRM Activity' then 1 else 0
                    end
                ...
                sum(
                    case
                        when bizible_marketing_channel = 'Organic Search' then 1 else 0
                    end
                ...
                sum(
                    case
                        when bizible_marketing_channel = 'Web Referral' then 1 else 0
                    end
                ...
                sum(
                    case
                        when crm_person_title = 'Development Team Lead' then 1 else 0
                    end
        ...
        product_usage as (
            ...
            where
                ...
                between date_trunc(
                    ...
                    month, dateadd(month, -1, cast('{{ end_date }}' as date))
                )
            group by dim_crm_account_id
    ...
    select
        ...
        coalesce(
            p1.parent_crm_account_sales_segment, 'Unknown'
        ) as account_sales_segment,
        ...
        -- , COALESCE(p1.parent_crm_account_tsp_region, 'Unknown') AS
        -- parent_account_region
        coalesce(
        ...
        - coalesce(license_count_prev, 0) as license_change_cnt,
        case
        ...
        coalesce(
            o.won_opportunities_new_business, 0
        ) as won_opportunities_new_business_cnt,
        coalesce(
            o.won_opportunities_add_on_business, 0
        ) as won_opportunities_add_on_business_cnt,
        coalesce(
            o.lost_opportunities_by_renewal, 0
        ) as lost_opportunities_by_renewal_cnt,
        coalesce(
        ...
        coalesce(
            o.competitors_github_enterprise, 0
        ) as competitors_github_enterprise_flag,
        ...
        case
            when o.account_id is not null then 1 else 0
        end as has_sfdc_opportunities_flag,
        -- ZoomInfo Fields
        ...
        coalesce(
            zt.zi_github_flag, zt.zi_github_enterprise_flag, 0
        ) as zi_github_any_flag,
        coalesce(
        ...
        coalesce(
            b.touchpoint_source_crm_campaign, 0
        ) as touchpoint_source_crm_campaign_cnt,
        coalesce(b.touchpoint_source_marketo, 0) as touchpoint_source_marketo_cnt,
        coalesce(
            b.touchpoint_source_crm_activity, 0
        ) as touchpoint_source_crm_activity_cnt,
        coalesce(
            b.touchpoint_source_social_media, 0
        ) as touchpoint_source_social_media_cnt,
        ...
        coalesce(
            b.touchpoint_bizible_medium_web, 0
        ) as touchpoint_bizible_medium_web_cnt,
        coalesce(
        ...
        u.projects_with_repositories_enabled_avg
        as projects_with_repositories_enabled_avg,
        ...
        u.template_repositories_all_time_event
        as template_repositories_all_time_event_cnt,
        ...
        case
            when u.dim_crm_account_id is not null then 1 else 0
        end as has_usage_data_flag
